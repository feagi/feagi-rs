//! System test: register a frame-based agent and stream frames to FEAGI.
//!
//! Usage:
//! 1) Ensure FEAGI is running with a loaded genome.
//! 2) Put PNG/JPG/BMP/TIFF frames in a directory.
//! 3) Run this test with the required environment variables.
//!
//! Example:
...
//! cargo test --test system_frame_agent -- --ignored --nocapture
//!
//! Required environment variables:
//! - FEAGI_TEST_FRAME_DIR (path to frame directory)
//! - FEAGI_TEST_FRAME_LOOPS (number of times to loop frames)
//! - FEAGI_TEST_GAZE_X (0.0-1.0)
//! - FEAGI_TEST_GAZE_Y (0.0-1.0)
//! - FEAGI_TEST_GAZE_MODULATION (0.0-1.0)
...
/// Test-specific settings sourced from environment variables.
struct TestSettings {
    agent_id: String,
    cortical_unit_id: u8,
...
/// Require and parse an environment variable into a typed value.
fn require_env_parse<T>(name: &str) -> Result<T>
where
    T: FromStr,
...
fn load_test_settings() -> Result<TestSettings> {
    ...
    Ok(TestSettings {
        ...
        frame_loops: require_env_parse("FEAGI_TEST_FRAME_LOOPS")?,
        gaze_x: require_env_parse("FEAGI_TEST_GAZE_X")?,
        gaze_y: require_env_parse("FEAGI_TEST_GAZE_Y")?,
        gaze_modulation: require_env_parse("FEAGI_TEST_GAZE_MODULATION")?,
    })
}
...
/// Load a fixed number of frames and validate consistent dimensions.
...
/// Build the agent configuration from FEAGI config and test settings.
fn build_agent_config(
    ...
    let tick_hz = (1.0 / timestep) as u32;
    if tick_hz == 0 {
        return Err(anyhow::anyhow!(
            "Derived feagi_tick_hz must be > 0 from burst_engine_timestep"
        ));
    }

    let service_startup_s = config.timeouts.service_startup;
    if service_startup_s <= 0.0 {
        return Err(anyhow::anyhow!(
            "config.timeouts.service_startup must be > 0"
        ));
    }
    ...
    let service_startup_ms = service_startup_s * 1000.0;
...
/// Register the vision device for this test using the SDK connector cache.
...
/// Run the system test to register and stream frames.
#[test]
#[ignore = "Requires running FEAGI instance and configured test frame sequence"]
fn system_frame_agent_streams_frames() -> Result<()> {
